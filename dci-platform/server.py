...
def init_db():
    ...
    # Pre-seed Demo Detective Account if DEMO-OFFICER doesn't exist
    cursor.execute("SELECT id FROM detectives WHERE callsign = 'DEMO-OFFICER'")
    if not cursor.fetchone():
        demo_pwd = hash_password("demo123")
        demo_token = "demo_static_token_9999"
        cursor.execute("INSERT INTO users (email, password_hash, token) VALUES ('demo.officer@dci.internal', ?, ?)", (demo_pwd, demo_token))
        user_id = cursor.lastrowid
        cursor.execute('''
            INSERT INTO detectives (user_id, detective_id, callsign, display_name, country, style, rank, clearance_level, xp, reputation, cases_solved, cases_completed_count)
            VALUES (?, 'DCI-26-999999', 'DEMO-OFFICER', 'Inspector Demo Officer', 'Global Operations', 'Analytical', 'SENIOR DETECTIVE', 4, 4500, 'EXEMPLARY', 2, 2)
        ''', (user_id,))
